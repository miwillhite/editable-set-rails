...
require File.expand_path(File.join(File.dirname(__FILE__), '../lib/editable_set'))
# Requires supporting files with custom matchers and macros, etc,
# in ./support/ and its subdirectories in alphabetic order.
Dir["#{File.dirname(__FILE__)}/support/**/*.rb"].sort.each {|f| require f}

module EditableSetSpecHelper
  include ActionPack
  ...
  def with_config(config_method_name, value, &block)
    old_value = EditableSetFormBuilder.send(config_method_name)
    EditableSetFormBuilder.send(:"#{config_method_name}=", value)
    yield
    EditableSetFormBuilder.send(:"#{config_method_name}=", old_value)
  end
